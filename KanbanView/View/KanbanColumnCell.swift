...
extension KanbanColumnCell: UIDragInteractionDelegate{
    ...
    func dragInteraction(_ interaction: UIDragInteraction, itemsForBeginning session: UIDragSession) -> [UIDragItem] {
        // Write Some Code Here
        let currentDragString: String = rows.reduce("kanban Current Backlog") { (prev, curr) -> String in
            return "\(prev) \n- \(curr.title)"
        }
        let stringItemProvider = NSItemProvider(object: currentDragString as NSString)
